...
function mergeData(local, remote){
  ...
  out.students = out.students.filter(s => !seenSt.has(s.name) && seenSt.set(s.name, 1));
  const seenSl = new Set();
  out.slots = out.slots.filter(sl => { const k = sl.studentId+'|'+sl.dow+'|'+sl.time; if (seenSl.has(k)) return false; seenSl.add(k); return true; });
  const seenEx = new Set();
  out.extras = out.extras.filter(ex => { const k = ex.studentId+'|'+ex.date+'|'+ex.time; if (seenEx.has(k)) return false; seenEx.add(k); return true; });
  return out;
}
